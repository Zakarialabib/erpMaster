<nav class="hidden md:flex gap-10 justify-end w-full" x-data="{ isDropdownOpen: false }">
    @foreach (\App\Helpers::getHeaderMenu() as $index => $item)
        @if ($item->type === 'link')
            <a href="{{ $item->url }}" @if ($item->new_window) target="__blank" @endif
                class="text-base font-bold text-red-950 hover:text-red-400 hover:underline focus:underline uppercase">
                @if ($item->icon)
        ...
        @elseif ($item->type === 'megamenu')
            <div class="relative" x-data="{ isOpen: false }">
                <button @click="isOpen = !isOpen"
                    class="text-base font-bold text-red-950 hover:text-red-300  hover:underline focus:underline uppercase">
                    @if ($item->icon)
                        <i class="{{ $item->icon }} mr-2"></i>
                    @endif
                    {{ $item->label }}
