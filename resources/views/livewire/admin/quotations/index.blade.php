<div>
    ...
    <x-theme.breadcrumb :title="__('Quotations List')" :parent="route('admin.quotations.index')" :parentName="__('Quotations List')">

        @can('quotation_create')
            <x-button href="{{ route('admin.quotation.create') }}" primary>
                {{ __('Create Quotation') }}
            </x-button>
        @endcan
        ...
        <x-table.tbody>
            @forelse ($quotations as $quotation)
                <x-table.tr wire:loading.class.delay="opacity-50">
                    ...
                    <x-table.td>
                        <a href="{{ route('admin.customer.details', $quotation->customer->id) }}"
                    ...
                    <x-table.td>
                        ...
                        <x-badge :type="$badgeType">{{ $quotation->status->label() }}</x-badge>
                    </x-table.td>
                    <x-table.td class="whitespace-no-wrap row-action--icon">
                        <x-dropdown align="right" width="56">
                            ...
                            <x-slot name="content">
                                @can('quotation_sale')
                                    <x-dropdown-link href="{{ route('admin.quotation-sales.create', $quotation) }}">
                                        <i class="fas fa-shopping-cart mr-2"></i>
                                        {{ __('Make Sale') }}
                                    </x-dropdown-link>
                                @endcan
                                @can('send_quotation_mails')
                                    <x-dropdown-link href="{{ route('admin.quotation.email', $quotation) }}">
                                        <i class="fas fa-envelope mr-2"></i>
                                        {{ __('Send Mail') }}
                                    </x-dropdown-link>
                                @endcan
                                @can('quotation_update')
                                    <x-dropdown-link href="{{ route('admin.quotation.edit', $quotation->id) }}">
    ...
    <livewire:admin.quotations.show :quotation="$quotation" lazy />
