                        ...
                        <div class="flex flex-wrap -mx-2 mb-3">
                            ...
                            <div class="w-full px-3 my-2">
                                ...
                                <x-media-upload title="{{ __('Product Image') }}" name="image" wire:model="image"
                                    :file="$image" :preview="$this->product-image" single types="PNG / JPEG / WEBP"
                                    fileTypes="image/*" />
                            </div>
                            ...
                            <div class="w-full px-3 my-2">
                                ...
                                <x-media-upload title="{{ __('Gallery') }}" name="gallery" wire:model="gallery"
                                    :preview="$this->product->gallery[]" :file="$gallery" multiple types="PNG / JPEG / WEBP"
                                    fileTypes="image/*" />
                            </div>
                        </div>
                                            ...
                                            <div class="flex flex-row w-full items-center space-x-4">
                                                <select wire:model.lazy="options.{{ $index }}.type"
                                                    ...
                                                    <option value="movementType"
                                                        {{ $option['type'] == 'movementType' ? 'selected' : '' }}>
                                                        {{ __('Movement type') }}</option>
                                                    <option value="waterproof"
                                                        {{ $option['type'] == 'waterproof' ? 'selected' : '' }}>
                                                        {{ __('Water Resiste') }}</option>
                                                    <option value="displayType"
                                                        {{ $option['type'] == 'displayType' ? 'selected' : '' }}>
                                                        {{ __('Display type') }}</option>
                                                    <option value="bandColor"
                                                        {{ $option['type'] == 'bandColor' ? 'selected' : '' }}>
                                                        {{ __('Band color') }}</option>
                                                    <option value="materials"
                                                        {{ $option['type'] == 'materials' ? 'selected' : '' }}>
                                                        {{ __('Materials') }}</option>
                                                    <option value="size"
                                                        {{ $option['type'] == 'size' ? 'selected' : '' }}>
                                                        {{ __('Size') }}
                                                    </option>
                                                </select>
                                                @if ($option['type'] === 'color')
                                                    <input type="color"
                                                        wire:model.lazy="options.{{ $index }}.value">
                                                @else
                                                    <input type="text"
                                                        wire:model.lazy="options.{{ $index }}.value">
                                                @endif
                                                <x-button danger type="button"
                                                    wire:click="removeOption({{ $index }})">{{ __('Remove') }}
                                                </x-button>
                                            </div>
                                        @endforeach
