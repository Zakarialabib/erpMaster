            ...
            <form wire:submit="update">
                <div class="grid grid-cols-2 gap-4 px-2">
                    ...
                    <div>
                        ...
                        {{-- @dd(App\Enums\PageType::cases()) --}}
                        <select wire:model.live="type" class="block w-full px-2 py-1 border border-gray-300 rounded-md">
                            <option value="">
                                {{ __('Select a Type') }}
                            </option>
                            @foreach (App\Enums\PageType::cases() as $key => $pagetype)
                                <option value="{{ $pagetype->value }}" @if ($type === $pagetype->value) selected @endif>
                                    {{ $pagetype->value }}
                                </option>
                            @endforeach
                        </select>
