    ...
    <form wire:submit="update">
        <div class="flex flex-wrap -mx-2 mb-3">
            ...
            <div class="w-full md:w-1/3 px-3 mb-4 md:mb-0">
                ...
                <x-input type="text" wire:model.live="mail_mailer" name="mail_mailer" />
            </div>
            <div class="w-full md:w-1/3 px-3 mb-4 md:mb-0">
                <label for="mail_username">{{ __('MAIL USERNAME') }}</label>
                <x-input type="text" wire:model.live="mail_username" name="mail_username" />
            </div>
            <div class="w-full md:w-1/3 px-3 mb-4 md:mb-0">
                <label for="mail_password">{{ __('MAIL PASSWORD') }}</label>
                <x-input type="password" wire:model.live="mail_password" name="mail_password" />
            </div>

            <div class="w-full md:w-1/3 px-3 mb-4 md:mb-0">
                <label for="mail_encryption">{{ __('MAIL ENCRYPTION') }}</label>
                <x-input type="text" wire:model.live="mail_encryption" name="mail_encryption" />
            </div>
            <div class="w-full md:w-1/3 px-3 mb-4 md:mb-0">
                <label for="mail_from_address">{{ __('MAIL FROM ADDRESS') }}</label>
                <x-input type="email" wire:model.live="mail_from_address" name="mail_from_address" />
            </div>
            <div class="w-full md:w-1/3 px-3 mb-4 md:mb-0">
                <label for="mail_from_name">{{ __('MAIL FROM NAME') }} <span class="text-red-500">*</span></label>
                <x-input type="text" wire:model.live="mail_from_name" name="mail_from_name" required />
            </div>
        </div>
