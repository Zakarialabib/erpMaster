        ...
        <form wire:submit="store">
            <div class="flex flex-wrap -mx-2 mb-3">
                <div class="xl:w-1/2 lg:w-1/2 sm:w-full px-3">
                    ...
                    <x-input type="text" wire:model.blur="reference" name="reference" required />
                </div>
                <div class="xl:w-1/2 lg:w-1/2 sm:w-full px-3">
                    <x-label for="date" :value="__('Date')" required />
                    <x-input type="date" wire:model.blur="date" name="date" required />
                </div>
            </div>
                    ...
                    <x-table.tbody>
                        @foreach ($products as $key => $product)
                            <x-table.tr wire:loading.class.delay="opacity-50">
                                ...
                                <x-table.td>
                                    <select name="types[]" wire:model.blur="products.{{ $key }}.types"
                                        class="block w-full shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm border-gray-300 rounded-md mt-1">
                                        <option value="add"
