<div>
    <x-modal wire:model.live="loginModal">
        <x-slot name="title">
            ...
            <form wire:submit="authenticate">
                <div class="w-full flex flex-wrap justify-center align-center py-10 px-4">
                    <div class="w-full px-2 mt-4">
                        ...
                        <select wire:model.blur="type" id="type" name="type"
                    ...
                    <div class="w-full px-2 mt-4">
                        ...
                        <x-input id="store_url" class="block mt-1 w-full" type="text" name="store_url"
                            wire:model.blur="store_url" required />
                        <x-input-error :messages="$errors->get('store_url')" for="store_url" class="mt-2" />
                    </div>

                    <div class="w-full px-2 mt-4">
                        <label for="email">{{ __('Email') }}</label>
                        <x-input type="email" id="email" required wire:model.blur="email" />
                        <x-input-error :messages="$errors->get('email')" class="mt-2" />
                    </div>
                    <div class="w-full px-2 mt-4">
                        <label for="password">{{ __('Password') }}</label>
                        <x-input type="password" id="password" required wire:model.blur="password" />
