<div>
    ...
    <x-theme.breadcrumb :title="__('Deliveries List')" :parent="route('admin.deliveries.index')" :parentName="__('Deliveries List')">
        <x-button primary type="button" wire:click="dispatchTo('admin.delivery.create', 'createModal')">
            {{ __('Create Delivery') }}
        </x-button>
    </x-theme.breadcrumb>
    ...
    <x-table>
        <x-slot name="thead">
            ...
            <x-table.th sortable :direction="$sorts['use_id'] ?? null" field="use_id" wire:click="sortingBy('use_id')">
                {{ __('User') }}
            </x-table.th>
        ...
        <x-table.tbody>
            @forelse ($deliveries as $delivery)
                <x-table.tr wire:loading.class.delay="opacity-50" wire:key="row-{{ $delivery->id }}">
                    <x-table.td class="pr-0">
                        <input wire:model.live="selected" type="checkbox" value="{{ $delivery->id }}" />
                    </x-table.td>
                    <x-table.td>
                    ...
                    <x-table.td>
                        ...
                        @else
                            {{ __('Sale') }}: <small>{{ $delivery->sale->customer->name ?? '' }}</small> |
                            <small>{{ $delivery->sale->customer->phone ?? '' }}</small>
                        @endif
                    </x-table.td>
                    <x-table.td>
                        @php
                            $type = $delivery->status->getBadgeType();
                        @endphp
                        <x-badge :type="$type">{{ $delivery->status->label() }}</x-badge>
                    </x-table.td>
                    <x-table.td>
                        {{ $delivery->created_at }}
                    </x-table.td>
                    <x-table.td>
                        <div class="flex justify-start space-x-2">
                            ...
                            <x-button danger wire:click="$dispatch('deleteModal', {{ $delivery->id }})" type="button"
                                wire:loading.attr="disabled">
                                <i class="fas fa-trash"></i>
                            </x-button>
                        </div>
                ...
                <x-table.tr>
                    <x-table.td colspan="9">
                        <div class="flex justify-center items-center space-x-2">
                            ...
                            <span
                                class="font-medium py-8 text-gray-400 text-xl">{{ __('No deliveries found...') }}</span>
                        </div>
                    </x-table.td>
                </x-table.tr>
            @endforelse
